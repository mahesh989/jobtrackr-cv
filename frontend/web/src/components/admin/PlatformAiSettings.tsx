            ...
            {isOpen && (
              <div className="border-t border-[var(--border)] bg-[var(--surface)] px-4 py-4 space-y-4">
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  {state.hasKey && !replaceMode[id] && (
                    <button onClick={() => setReplaceMode((p) => ({ ...p, [id]: true }))} className="gh-btn text-[12px]">
                      Replace key
                    </button>
                  )}
                  ...
                  {state.hasKey && !state.isActive && (
                    ...
                    >
                      {activating === id
                        ? <><Loader2 className="w-3 h-3 animate-spin inline mr-1" />Activating…</>
                        : "Make active for all users"}
                    </button>
                  )}
                </div>
              </div>
