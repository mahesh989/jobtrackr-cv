...
import { generateInviteCode, revokeInviteCode } from "@/lib/actions/invites";
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <StatCard label="AI error rate (today)"
              ...
              href="/admin/pipeline" color={errorCalls > 3 ? "red" : "green"} />
            <StatCard label="p95 latency (today)"
              value={p95Latency !== null ? `${(p95Latency / 1000).toFixed(1)}s` : "—"}
              sub="per AI call" href="/admin/pipeline"
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <StatCard label="Trialing"
              ...
              sub="on free trial" href="/admin/revenue" color="amber" />
            <StatCard label="Past due"
              value={String(subs.filter((s) => s.status === "past_due").length)}
              sub="payment failing" href="/admin/revenue"
              color={subs.some((s) => s.status === "past_due") ? "red" : "slate"} />
            <StatCard label="Time to first value" href="/admin/retention"
              value={ttvLabel}
