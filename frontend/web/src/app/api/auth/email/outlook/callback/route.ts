...
export async function GET(req: NextRequest) {
  ...
  if (error || !code) {
    return NextResponse.redirect(
      `${appUrl}/settings/account?email_error=${encodeURIComponent(error ?? "no_code")}`,
    );
  }
  ...
  if (!storedState || storedState !== state) {
    return NextResponse.redirect(
      `${appUrl}/settings/account?email_error=invalid_state`,
    );
  }
  ...
  if (!tokenRes.ok) {
    ...
    console.error("[outlook/callback] token exchange failed:", tokenRes.status, body);
    return NextResponse.redirect(
  ...
  if (!access_token || !refresh_token) {
    return NextResponse.redirect(
      `${appUrl}/settings/account?email_error=missing_tokens`,
    );
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      `${appUrl}/settings/account?email_error=${encodeURIComponent("save_failed:" + msg.slice(0, 120))}`,
    );
  }

  return NextResponse.redirect(
    `${appUrl}/settings/account?email_connected=outlook`,
  );
}
