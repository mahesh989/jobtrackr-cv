...
export default async function PrivacyPage() {
  ...
  // Logged-in visitors (e.g. via the sidebar "Privacy policy" link) get the
  // real app chrome so this doesn't feel like a jump out of the product.
  // Anonymous visitors (landing page footer, /terms cross-link) never hit
  // auth here — middleware.ts keeps /privacy public on purpose.
  if (user) {
      ...
      <ThemeProvider>
        <div className="flex h-screen overflow-hidden bg-[var(--sidebar-bg)]">
          ...
          <div className="flex-1 flex flex-col min-w-0 bg-bg overflow-y-auto">
            <Header />
            <PrivacyContent backHref="/dashboard" backLabel="Back to dashboard" />
          </div>
        </div>
      </ThemeProvider>
    );
  ...
  return (
    <main className="min-h-screen bg-bg text-text-2">
      ...
      <PrivacyContent backHref="/" backLabel="Back to home" />
    </main>
  );
}
