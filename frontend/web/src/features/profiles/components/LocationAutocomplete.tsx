    ...
    <div ref={wrapRef} className="relative">
      ...
      {open && (
        <ul className="absolute z-20 mt-1 w-full max-h-60 overflow-auto rounded-md border border-border bg-surface shadow-lg">
          {suggestions.map((s, i) => (
            <li key={s}>
              <button type="button" onMouseDown={(e) => { e.preventDefault(); select(s); }} onMouseEnter={() => setActive(i)} className={`block w-full px-3 py-2 text-left text-body text-text ${ i === active ? "bg-brand/10" : "" }`}>
                {s}
              </button>
            </li>
          ))}
