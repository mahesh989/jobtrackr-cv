  ...
  return (
    <div className="flex flex-nowrap sm:flex-wrap items-center gap-2 px-5 pb-4 pt-1 border-t border-border overflow-x-auto sm:overflow-visible whitespace-nowrap sm:whitespace-normal">
      ...
      {callouts.passedButNoLetter > 0 && (
        <FilterAnchor href="/?triage=passedNoLetter" shallow={shallow} apply={applyFilter} className="inline-flex shrink-0 items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-medium bg-blue-50 border border-blue-200 text-blue-700 hover:bg-blue-100 transition-colors">
          → {callouts.passedButNoLetter} passed ATS, no letter yet
        </FilterAnchor>
      )}
      {callouts.readyToApply > 0 && (
        <Link href="/applications" className="inline-flex shrink-0 items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-medium bg-pink-50 border border-pink-200 text-pink-700 hover:bg-pink-100 transition-colors">
