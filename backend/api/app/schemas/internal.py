...
class AnalyzeEvalRequest(BaseModel):
    """
    Run ONE (writer × scorer) variant on a pasted CV+JD, synchronously.

    Used only by the founder-only beta screen. No job record, no analysis_runs
    write — results go to the isolated eval_runs table. BYOK like /analyze.
    """
    cv_text:        str = Field(min_length=1)
    jd_text:        str = Field(min_length=1)

    writer_variant: str = "w1_current"
    scorer_variant: str = "s1_current"

    # provenance / grouping (all optional)
    ...
    # BYOK — per-request, never persisted.
    ai_provider:    Literal["anthropic", "openai", "deepseek"]
    ai_api_key:     str = Field(min_length=1)
    ai_model:       Optional[str] = None


class AnalyzeEvalResponse(BaseModel):
    """
    Trigger response for /internal/analyze-eval. The eval runs as a background
    task; the web polls /internal/eval-run/{id} until status='completed'.
    """
    eval_run_id: str
...
class EvalRunResponse(BaseModel):
    """
    Full state of an eval_runs row. Returned by GET /internal/eval-run/{id}.
    All result fields are null until status flips to 'completed'.
    """
    ...
    tailored_md:        Optional[str] = None
    initial_ats:        Optional[int] = None
    final_ats:          Optional[int] = None
    ats_lift:           Optional[int] = None

    structural_summary: Optional[Dict[str, Any]] = None
    grounding_report:   Optional[Dict[str, Any]] = None
    rescore_report:     Optional[Dict[str, Any]] = None
