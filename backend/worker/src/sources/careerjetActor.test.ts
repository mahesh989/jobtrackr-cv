 ...
 * enrichCareerjetJDsViaActor billed the FULL run cost and reported
...
function careerjetJob(url: string): NormalisedJob {
  return {
    url,
    url_hash: "", content_hash: "",
    title: "Registered Nurse", company: "Acme", location: "Sydney",
    description: "teaser", source: "careerjet", source_tier: 1,
...
const JOBS = [careerjetJob("https://www.careerjet.com.au/jobad/123")];

describe("enrichCareerjetJDsViaActor — failure billing", () => {
  beforeEach(() => {
    process.env.CAREERJET_ACTOR_ID = "test-actor";
    vi.resetModules();
  });

  afterEach(() => {
    vi.restoreAllMocks();
    delete process.env.CAREERJET_ACTOR_ID;
  });
  ...
  it("REGRESSION: reports zero cost and zero fetched on a non-ok Apify response, instead of billing a failed run as a full success", async () => {
    ...
    const { enrichCareerjetJDsViaActor } = await import("./careerjetActor.js");

    const result = await enrichCareerjetJDsViaActor(JOBS, "token");
  ...
  it("still bills the run cost + reports fetched on a genuine success", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => ({
      ok: true, status: 200,
      json: async () => [{ url: "https://www.careerjet.com.au/jobad/123", description: "x".repeat(300) }],
    } as Response)));
    const { enrichCareerjetJDsViaActor } = await import("./careerjetActor.js");
