...
_STOPCAPS = {
    ...
    "tools", "responsibilities", "achievements",
}
...
def compute_grounding(tailored_markdown: str, original_cv_text: str) -> Dict[str, Any]:
    ...
    for m in _MULTI_RE.finditer(tailored_markdown or ""):
    ...
    for m in _ACRO_RE.finditer(tailored_markdown or ""):
        token = m.group(1)
        if token.lower() in _STOPCAPS:
            continue
        candidates.append(token)

    for m in _SLASH_RE.finditer(tailored_markdown or ""):
        candidates.append(m.group(1))

    candidates.extend(_single_word_candidates(tailored_markdown or ""))
