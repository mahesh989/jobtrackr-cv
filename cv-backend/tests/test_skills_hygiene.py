...
# ---------------------------------------------------------------------------
# Cap-aware swap regression (the Rashmi/Nepean Private Hospital case).
# Sprint F lowered the Soft Skills cap to 6. The previous injector appended
# beyond the cap (SURFACE_CAPS=8), then a final enforce_skills_section in the
# writer truncated the just-appended approved keywords back to 6 — so all 5
# approved soft skills (customer service, verbal/written communication,
# work planning/prioritisation) silently vanished from the tailored CV.
# These tests lock in the cap-aware swap-out so the injector survives the cap.
# ---------------------------------------------------------------------------


def test_writer_only_displaced_to_make_room_for_approved():
    """Soft Skills line at cap=6: 4 writer items are approved, 2 are writer-only
    (Time Management, De-Escalation Techniques). 2 approved keywords need to
    land — the writer-only items must be displaced, NOT the approved ones.
    """
    md = (
        "## Skills\n"
        "**Care Skills:** Personal Care, Dementia Care\n"
        "**Soft Skills:** Teamwork, Time Management, De-Escalation Techniques, "
        "Flexibility, Reliability, Caring Attitude\n"
        "**Other Skills:** BESTMed, MedMobile\n\n"
        "## Experience\n"
    )
    feas = _feasibility(
        ("customer service",      "soft_skills", "inject_as_extension"),
        ("verbal communication",  "soft_skills", "inject_as_extension"),
        # The 4 writer items already in the JD's approved list:
        ("teamwork",              "soft_skills", "inject_directly"),
        ("flexibility",           "soft_skills", "inject_directly"),
        ("reliability",           "soft_skills", "inject_directly"),
        ("caring attitude",       "soft_skills", "inject_directly"),
    )
    out = _inject_approved_skills(md, feas)
    soft_line = next(ln for ln in out.split("\n") if "**Soft Skills:**" in ln)
    # New approved keywords must land
    assert "Customer Service" in soft_line
    assert "Verbal Communication" in soft_line
    # Approved writer items must survive (in approved bucket → preserved)
    for kw in ("Teamwork", "Flexibility", "Reliability", "Caring Attitude"):
        assert kw in soft_line, f"approved writer item ejected: {kw}"
    # Non-approved writer items (not in any inject bucket) MUST be displaced
    assert "Time Management" not in soft_line
    assert "De-Escalation Techniques" not in soft_line


def test_soft_skills_line_at_or_below_cap_after_inject():
    """Cap-aware: the Soft Skills line (at position 1 → DEFAULT_SKILL_CAPS[1]=6)
    must end ≤ 6 items even when the feasibility plan lists 10 approved
    keywords. No follow-up enforce pass is needed to trim.
    """
    md = (
        "## Skills\n"
        "**Care Skills:** Personal Care, Dementia Care\n"
        "**Soft Skills:** A, B, C, D, E, F\n"
        "**Other Skills:** X, Y\n\n"
        "## Experience\n"
    )
    feas = _feasibility(*[
        (f"approved {i}", "soft_skills", "inject_directly") for i in range(10)
    ])
    out = _inject_approved_skills(md, feas)
    soft_line = next(ln for ln in out.split("\n") if "**Soft Skills:**" in ln)
    items = [s.strip() for s in soft_line.split("**Soft Skills:**", 1)[1].split(",")]
    items = [s for s in items if s]
    assert len(items) <= 6, f"line over cap: {items}"


def test_one_approved_lands_when_writer_has_non_approved_tail():
    """When writer-only items occupy tail slots and one approved keyword is
    pending, the writer-only items must yield first — the new approved one
    must land within the cap. Soft Skills at position 1 → cap=6.
    """
    md = (
        "## Skills\n"
        "**Care Skills:** Personal Care, Dementia Care\n"
...
def test_no_followup_enforce_needed_after_inject():
    """The injector's output is its own final shape — wrapping with
    enforce_skills_section must NOT change/truncate the approved keywords
    we placed. Regression for the previous bug where the final enforce
    silently dropped the injected keywords from the line tail.
    """
    md = (
        "## Skills\n"
        "**Soft Skills:** Teamwork, Time Management, De-Escalation Techniques, "
    ...
    feas = _feasibility(
        ("verbal communication",  "soft_skills", "inject_as_extension"),
        ("written communication", "soft_skills", "inject_as_extension"),
    )
    injected = _inject_approved_skills(md, feas)
    after_enforce = enforce_skills_section(injected)
    for kw in ("Verbal Communication", "Written Communication"):
        assert kw in after_enforce, (
            f"approved keyword '{kw}' was lost when enforce_skills_section ran "
            "after the injector — the writer must NOT run enforce after inject."
        )
