...
|---|---|---|---|
| **web** | Next.js 16.2.6 / React 19 / TypeScript | `@supabase/ssr`, `@supabase/supabase-js`, BullMQ, Tailwind v4 | Vercel |
| **cv-backend** | Python 3.11 / FastAPI 0.115.5 / Uvicorn | `anthropic 0.97`, `openai 1.109`, `reportlab 4.2.2`, `pypdf 5.1.0`, `python-docx 1.1.2` | Fly.io (`jobtrackr-cv-api`, region: `syd`) |
| **worker** | Node.js / BullMQ ^5.13 / ioredis ^5.4 | Apify actors in `worker/apify-actors/` | Fly.io (`jobtrackr-cv-worker`) |
| **database** | Supabase Postgres | Supabase Auth, RLS, Realtime, Storage | Supabase (shared with production JobTrackr) |

cv-backend Fly.io machine spec: `shared-cpu-1x`, 512 MB RAM, `min_machines_running = 1` (kept warm to avoid cold-start timeouts on Vercel Hobby's 10 s function limit).
...
│       └── lib/
│           └── cvBackend.ts    # HMAC-signed caller to cv-backend (server-only)
├── cv-backend/                 # FastAPI pipeline service (Fly.io)
│   └── app/
│       ├── routes/internal.py  # /internal/* endpoints (HMAC-protected)
...
│       └── fly.toml
├── worker/                     # Job-discovery worker (Fly.io, unchanged from JobTrackr)
├── supabase/migrations/        # 20 SQL migration files (manually applied, CLI-untracked)
└── docs/                       # This folder
```
...
cv-backend never sees user sessions. It receives `user_id` as a plain field in HMAC-signed request payloads and writes directly to Supabase using the **service-role key** (bypasses RLS — intentional for pipeline writes).

---

## HMAC Boundary (web ↔ cv-backend)

cv-backend has **no public auth surface** — it is Fly.io private networking only. Every call from Next.js API routes must be signed.

**Signing (web/src/lib/cvBackend.ts):**
```
X-Timestamp: <unix-seconds>
X-Signature: HMAC-SHA256(JOBTRACKR_HMAC_SECRET, timestamp_string + raw_json_body)
```

Encoding: the timestamp is concatenated as its ASCII decimal string representation (not packed bytes), followed by the raw JSON body bytes. Both sides must agree on this — see `web/src/lib/cvBackend.ts` and `cv-backend/app/security/hmac.py` for the canonical implementation.

**Verification (cv-backend/app/security/hmac.py):**
- Missing headers → 401
...
Users store their own Anthropic / OpenAI / DeepSeek API keys, encrypted AES-256-GCM in `user_integrations`. The web layer decrypts and passes the plaintext key to cv-backend in each HMAC-signed request body — **keys never touch cv-backend's database**.

`cv-backend/app/services/ai/client.py` receives `(provider, api_key, model)` per-request and routes to the correct SDK. Falls back to provider default model if stored model fails or returns "not a chat model" error.

---

## Internal API Endpoints

All cv-backend endpoints require HMAC verification (`verify_hmac` FastAPI dependency):
...
|---|---|---|---|
| web | `jobtrackr-cv` (Vercel project) | auto | Push to `main` |
| cv-backend | `jobtrackr-cv-api` (Fly.io) | `syd` | `flyctl deploy` (manual) |
| worker | `jobtrackr-cv-worker` (Fly.io) | — | `flyctl deploy` (manual) |

**cv-backend deploys are manual** — `flyctl deploy` from `cv-backend/`. Code changes to cv-backend do not auto-deploy on git push.
...
3. **cv-backend stays Python** — ReportLab PDF output is not portable; all pipeline logic stays in cv-backend.
