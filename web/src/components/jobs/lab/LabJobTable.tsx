...
/**
 * Lab fork of /web/src/components/JobTable.tsx — DO NOT edit the
 * original. This file adds:
 *   1. A "Progress" column (4 inline lucide icons: Analysed, Tailored CV,
 *      Cover Letter, Applied). Filled when done, outlined otherwise.
 *      Tooltips on hover. Click → jump to the relevant artefact.
 *   2. Grid changed from grid-cols-12 to retain layout — Role shrinks
 *      from col-span-4 to col-span-3 to make room for the new column.
 *   3. Respects LabSettings.hideProgressOnDismissed.
 *
 * Row-level behaviour (expand on click, Apply/Dismiss flash+collapse,
 * Edit JD modal, ⋮ overflow menu, Visa badge) is identical to the
 * original — copied verbatim. When the lab is promoted to production,
 * fold these changes back into JobTable.tsx and delete this file.
 */
...
export interface LabJob {
  ...
  /** Derived in lab page.tsx via progressFlags.deriveProgress(). */
...
export function LabJobTable({ jobs, showVisa, currentTab }: {
  jobs:       LabJob[];
  showVisa:   boolean;
  currentTab: string;
}) {
...
function JobRow({ job, showVisa, animDelay, currentTab }: {
  ...
  async function handleApply(e: React.MouseEvent) {
    ...
    try { await markJobApplied(job.id, job.profile_id); }
      ...
      console.error("[LabJobRow] markJobApplied failed:", err);
  ...
  async function handleDismiss(e: React.MouseEvent) {
    ...
    try { await markJobDismissed(job.id, job.profile_id); }
      ...
      console.error("[LabJobRow] markJobDismissed failed:", err);
      setExitPhase("idle");
    } finally { setIsPending(false); }
  }
  ...
  return (
    ...
    >
      <div style={{ overflow: "hidden" }}>
        ...
        >
          {/* Role (col-span-3 — shrunk from 4 to make room for Progress) */}
...
// ── Progress icons (NEW) ─────────────────────────────────────────────────────
// Four inline icons in a fixed order. Filled when done, outlined otherwise.
// Hover tooltip explains state. Click on a "done" icon jumps to the artefact.
function ProgressIcons({ job, dimmed }: { job: LabJob; dimmed: boolean }) {
...
// ── ⋮ overflow menu — verbatim from original JobTable ─────────────────────────
function RowMenu({
...
// ── Visa badge — verbatim from original ──────────────────────────────────────
