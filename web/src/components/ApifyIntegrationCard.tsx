...
// ── Status badge — uses app badge classes ─────────────────────────────────────
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    valid:              { label: "Connected",     cls: "badge badge-green" },
    pending_validation: { label: "Validating…",   cls: "badge badge-amber" },
    quota_exceeded:     { label: "Quota reached", cls: "badge badge-amber" },
    invalid:            { label: "Invalid token", cls: "badge badge-red"   },
    expired:            { label: "Token expired", cls: "badge badge-red"   },
    revoked:            { label: "Token revoked", cls: "badge badge-red"   },
    disabled:           { label: "Disabled",      cls: "badge badge-gray"  },
  };
      ...
      <div className="px-5 py-5 space-y-5">
        ...
        {connected && data ? (
          ...
          <>
            ...
            {!showInput ? (
              <div className="flex items-center gap-2">
                <button onClick={() => setShowInput(true)} className="gh-btn text-[12px]">
                  Replace token
                </button>
                <button
    ...
    <div className="space-y-3">
      <div>
        ...
        <div className="relative">
          <input
            ...
            className="field pr-9 font-mono text-[13px]"
            autoComplete="off"
            spellCheck={false}
          />
          <button
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="gh-btn gh-btn-blue text-[12px]"
        >
          {isPending ? (
        ...
        {onCancel && (
          <button onClick={onCancel} className="gh-btn text-[12px]">Cancel</button>
        )}
