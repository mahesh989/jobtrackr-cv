/**
 * /dashboard/admin/activity — User activity feed
 *
      ...
      <div className="px-6 py-5 flex gap-6 max-w-7xl">
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Link
              href={filterUser ? `/dashboard/admin/activity?user=${filterUser}` : "/dashboard/admin/activity"}
            ...
            {eventTypes.map((et) => (
              <Link
                key={et}
                href={`/dashboard/admin/activity?${filterUser ? `user=${filterUser}&` : ""}event=${et}`}
          ...
          {filterUser && (
            <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-md px-3 py-2">
              ...
              <Link href="/dashboard/admin/activity" className="ml-auto text-[11px] text-blue-600 hover:underline">Clear filter</Link>
            </div>
          )}
                ...
                <div key={e.id ?? i} className="flex items-start gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <Link
                        href={`/dashboard/admin/activity?user=${e.user_id}`}
        ...
        <div className="w-64 shrink-0 hidden lg:block">
          ...
          <div className="space-y-1">
            {users.map((u) => (
              <Link
                key={u.id}
                href={`/dashboard/admin/activity?user=${u.id}`}
